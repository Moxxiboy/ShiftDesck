...
def schedule():
            ...
            for employee_id in employee_ids:
                ...
                if target_user_id:
                    notify_user(
                        ...
                        url_for("schedule", view="week", week_start=(datetime.strptime(work_date, "%Y-%m-%d").date() - timedelta(days=datetime.strptime(work_date, "%Y-%m-%d").date().weekday())).isoformat()),
                        conn
                    )
    ...
    if view == "week":
        try:
            display_start = datetime.strptime(week_start_param, "%Y-%m-%d").date() if week_start_param else today
        except Exception:
            display_start = today
    ...
    return render_template(
        ...
        can_send_payroll_report=bool(has_permission(user, "can_view_payroll_reports", conn=None) or is_team_manager(user) or is_director_or_above(user) or is_root_admin(user)),
        edit_mode=edit_mode,
        can_edit_schedule=bool(manageable_ids)
    )
